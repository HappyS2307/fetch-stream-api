...
const searchToonStream = async (query) => {
    ...
    // ToonStream has changed domains.
    const searchUrls = [
    ...
    for (const searchUrl of searchUrls) {
        ...
        try {
            ...
            // Fallback for newer ToonStream theme
            if (!results.length) {
...
// DramaWorld public-page adapter
...
const getDramaWorldEpisodes = async (rawUrl) => {
    ...
    // If supplied URL itself is an episode page
    if (!episodes.length) {
        ...
        $('script').each((i, el) => {
            ...
            if (scriptContent) {
                ...
                if (m3u8Match) {
                    ...
                    directVideoUrl =
                        m3u8Match[1].replace(/\\/g, '');

                } else if (mp4Match) {

                    directVideoUrl =
                        mp4Match[1].replace(/\\/g, '');
                }
            }
        });
...
app.get('/toonstream/streams', async (req, res) => {
    ...
    try {
        ...
        const resolvedStreams = await Promise.all(
            rawStreams.map(async (stream) => {
                if (stream.link.includes('/embed/')) {
                    const bypassedLink =
                        await resolveEmbedUrl(stream.link);

                    return {
                        server: stream.serverName,
                        link: bypassedLink,
                        is_bypassed:
                            bypassedLink !== stream.link
                    };
                }
        ...
        $('.cyber-modal .links-list .link-row').each((i, el) => {
            const serverName =
                $(el).find('.server-name').text().trim();

            const downloadLink =
                $(el).find('a.download-btn').attr('href');

            if (downloadLink) {
                downloadSources.push({
                    server:
                        serverName || `Mirror ${i + 1}`,
                    link: downloadLink
                });
            }
        });
...
app.get('/dramaworld/episodes', async (req, res) => {
    ...
    try {
        const episodes =
            await getDramaWorldEpisodes(rawUrl);
...
app.get('/dramaworld/streams', async (req, res) => {
    ...
    try {
        const result =
            await getDramaWorldStreams(rawUrl);
