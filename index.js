            ...
            if (!results.length) {
                $('a[href]').each((index, element) => {
                    ...
                    if (!title) {
                        ...
                        title = parent
                            .find('h2, h3, h4, .title, .film-name, .entry-title')
                            .first()
                            .text()
                            .trim();
                    }
    ...
    if (!episodes.length) {
        ...
        if (match) {
            episodes.push({
                epNum: match[1],
                title:
                    $('h1,h2,.entry-title').first().text().trim() ||
                    `Episode ${match[1]}`,
                link: pageUrl,
                image: null
            });
...
const getDramaWorldStreams = async (rawUrl) => {
    ...
    const title =
        $('h1,h2,.entry-title').first().text().trim() || null;
    ...
    $('a.download-link-btn[href], a[href*="/stream/download/"]')
        .each((i, el) => {
            ...
            const quality =
                $(el).find('.dl-quality').text().trim() || null;

            const server =
                $(el).find('.dl-name').text().trim() ||
                'Download';
...
app.get('/toonstream/streams', async (req, res) => {
    ...
    try {
        ...
        // Collect All Public iFrame Players
        $(
            'iframe, ' +
            '.video iframe, ' +
            '.video-player iframe'
        ...
        // Search Scripts for Public Media URLs
        $('script').each((i, el) => {
            const text =
                $(el).html() || '';
